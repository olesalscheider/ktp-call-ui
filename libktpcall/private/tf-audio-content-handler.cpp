...
bool TfAudioContentHandler::startSending()
{
    ...
    if (!src) {
        qCDebug(KTP_CALL_UI) << "Could not initialize audio capture device";
        return false;
    }
...
void TfAudioContentHandler::refSink()
{
    ...
    if (m_sinkRefCount++ == 0) {
        ...
        if (!m_sink) {
            qCWarning(KTP_CALL_UI) << "Failed to create audio sink. Using fakesink. "
        ...
        if (!m_outputAdder) {
            qCWarning(KTP_CALL_UI) << "Failed to create liveadder. Using adder. This may cause trouble...";
...
bool TfAudioContentHandler::createSrcBin(const QGst::ElementPtr & src)
{
    ...
    try {
        ...
        qCWarning(KTP_CALL_UI) << "Failed to create audio source bin" << err;
        return false;
    }
    ...
    if (!src->link(firstElement)) {
        qCWarning(KTP_CALL_UI) << "Failed to link audiosrc to audio src bin";
        return false;
    }
    ...
    if (!queue) {
        qCWarning(KTP_CALL_UI) << "Failed to load the 'queue' gst element";
        return false;
    }
    bin->add(queue);
