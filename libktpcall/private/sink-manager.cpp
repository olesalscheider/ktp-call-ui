...
void SinkManager::cleanup()
{
    ...
    qCDebug(KTP_CALL_UI) << m_controllers.size() << "pads need unlinking";
...
{
    qCDebug(KTP_CALL_UI) << "New src pad" << pad->name() << "from handle" << contactHandle;
...
void SinkManager::handleNewSinkPadAsync(uint contactHandle)
{
    ...
    if (Q_UNLIKELY(!ctrl)) {
        ...
        qCDebug(KTP_CALL_UI) << "Not handling new pad. The pad was unlinked too early.";
        return;
    }
...
void SinkManager::onPendingContactsFinished(Tp::PendingOperation *op)
{
    ...
    if (Q_UNLIKELY(pc->isError())) {
        qCCritical(KTP_CALL_UI) << "Failed to fetch contact for handle" << contactHandle
...
void SinkManager::onPadUnlinked(const QGst::PadPtr & srcPad)
{
    ...
    qCDebug(KTP_CALL_UI) << srcPad->name() << "was unlinked.";
    qCDebug(KTP_CALL_UI) << "Current thread:" << QThread::currentThread()
    ...
    if (Q_UNLIKELY(!ctrl)) {
        ...
        qCDebug(KTP_CALL_UI) << "Looks like unlinkAllPads() wins...";
        return;
    }
