    ...
    for (int i=0; i < indices.size(); ++i) {
        ...
        if (hideAdvancedDevices()) {
            ...
            if (var.isValid() && var.toBool()) {
                qCDebug(KTP_CALL_UI) << "hiding device" << indices.at(i)
        ...
        if (accessList.isValid()) {
            qCDebug(KTP_CALL_UI) << "appending device access list for device" << indices.at(i);
                                                                    ...
                                                                    Phonon::Category category)
{
    ...
    qCDebug(KTP_CALL_UI) << "got video device indices" << indices << "for type" << type;
